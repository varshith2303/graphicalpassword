...
export default function Signup(props) {
    ...
    // Flatten the selected image URLs or IDs (either works)
    const selectedIds = signupInfo.pattern
    const patternString = selectedIds.join("-") // You can also use URLs

    // Generate bcrypt hash
    const salt = await bcrypt.genSalt(10)
    const hashedPattern = await bcrypt.hash(patternString, salt)
    console.log(hashedPattern);
    ...
    const finalSignupInfo = {
        ...
        sets:signupInfo.sets
        
    }
    console.log("finalSignupInfo",finalSignupInfo);

    props.setLoading(true)
    axios.post(`${api.url}/api/user/signup`, finalSignupInfo)
        .then(res => {
        ...
        <div className=" sm:h-[38rem] mt-12">
            {!next && <div className="flex justify-center h-full">
                ...
                <div className="font-['Work_Sans'] mt-4">
                    ...
                    <div className="flex flex-col w-[80%] sm:w-2/3 px-4 sm:px-0">
                        <input value={signupInfo.username} onChange={handleChange} name="username" className="rounded-lg h-8 sm:h-12 px-6 font-3xl" type="text" placeholder="Username"/>
                        <input value={signupInfo.email} onChange={handleChange} name="email" className="rounded-lg h-8 sm:h-12 px-6 font-3xl mt-4" type="email" placeholder="Email"/>
                        <input value={signupInfo.password} onChange={handleChange} name="password" className="rounded-lg h-8 sm:h-12 px-6 font-3xl mt-4" type="password" placeholder="Password"/>
                    </div>
            ...
            {next && <div className="sm:flex h-full">
                ...
                <div className="hidden sm:block font-['Work_Sans'] mt-4 ml-12">
                    ...
                    <button onClick={createAccount} className="transition duration-500 ease-in-out h-12 bg-[#A27B5C] rounded-lg px-6 w-2/3 mt-12 text-white border-2 hover:bg-transparent border-[#A27B5C] font-bold">{getButtonTitle()}</button>
                ...
                <div className="sm:hidden font-['Work_Sans'] mt-4 ml-4">
                    ...
                    <button onClick={createAccount} className="transition duration-500 ease-in-out h-8 bg-[#A27B5C] rounded-lg px-6 w-2/3 mt-12 text-white border-2 hover:bg-transparent border-[#A27B5C]">{getButtonTitle()}</button>
                    <button onClick={handleBackClick} className="transition duration-500 ease-in-out border-2 border-[#A27B5C] rounded-lg px-4 h-8 ml-4 hover:bg-[#A27B5C]">
                        <FontAwesomeIcon className="text-white" icon={faArrowLeft} />
                    </button>
                </div>
            </div>}
